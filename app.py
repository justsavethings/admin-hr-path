...
# Streamlit frontend for HR Document Delivery (direct DB lookup).

# UX rules implemented:
# - Single text input for 'Work Email' and 'Check Access' button.
# - Sanitizes input (strip + lower).
# - Uses chroma collection.get(ids=[email]) for O(1)/O(log n) lookup.
# - If email found: shows success banner, user card with metadata, and a download button
#   for 'employee-handbook.pdf' (handles missing file).
# - If not found: displays the exact warning message:
#   "Please delete the email – you don’t work with this company."
# - Uses st.cache_resource for the Chroma DB client.
# """
...
#     """Append a successful access event to `access_log.csv`.

#     Fields: timestamp, email, firstName, lastName, companyName, department, title, country, state
#     """
#     header = [
#         "timestamp",
#         "email",
...
#             writer.writerow(row)
#     except Exception:
#         # Fail silently in UI but could be logged elsewhere
#         pass
...
#     /* Buttons */
#     .stButton>button {
#         background: linear-gradient(180deg,#0366d6,#0356b6) !important;
...
#     except Exception:
#         # If collection doesn't exist, create an empty one (no data)
#         return client.create_collection(name)


# def display_user_card(metadata: dict):
#     # Improved, compact card-like display using columns
#     name = f"{metadata.get('firstName','').strip()} {metadata.get('lastName','').strip()}".strip()
#     company = metadata.get("companyName", "")
#     department = metadata.get("department", "")
#     title = metadata.get("title", "")
#     location_parts = [metadata.get("state", ""), metadata.get("country", "")]
#     location = ", ".join([part for part in location_parts if part])

#     col1, col2 = st.columns([2, 3])
#     with col1:
#         st.markdown("### " + (name if name else "—"))
#         st.caption(company if company else "—")
#     with col2:
#         st.write("**Title:**", title if title else "—")
#         st.write("**Department:**", department if department else "—")
#         st.write("**Location:**", location if location else "—")
#     st.divider()


# def main():
#     st.set_page_config(page_title="HR Document Delivery", layout="centered")
#     st.title("HR Document Delivery")

#     client = get_chroma_client()
#     collection = get_collection(client, COLLECTION_NAME)

#     st.write("Enter your work email to check access to the employee handbook.")
#     email_input = st.text_input("Work Email", value="", placeholder="name@company.com")
...
#         # Chroma get typically returns dict with keys: ids, metadatas, documents
#         ids = result.get("ids", []) if isinstance(result, dict) else []
#         if ids:
#             # Found
#             st.success("✅ Access granted")
#             # Extract metadata for the first matching id
#             metadatas = result.get("metadatas", [{}])
#             metadata = metadatas[0] if metadatas else {}
#             display_user_card(metadata)

#             # Provide download button for handbook
...
#         else:
#             # Not found => exact message required
